...
angular.module('akat-form-creator').directive('elemuiselect', ['$http', '$injector', '$q', '$compile', function ($http, $injector, $q, $compile) {
    return {
        ...
        link: function link($scope, element) {
            ...
            var repeat = [];
            var text = _.get($scope, 'options.selected', 'text');
            var request = false;
            ...
            match.html('{{ $select.selected.' + text + ' }}');
            ...
            if (_.get($scope, 'options.template', false)) {
                ...
                choices.html('<span ng-bind-html="item.' + text + ' | highlight: $select.search"></span>');
            }
            ...
            element.find('span').replaceWith($compile(select.append(match).append(choices))($scope));
        }
    };
}]);
