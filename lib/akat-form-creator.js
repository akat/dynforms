        ...
        link: function link($scope, element) {
            ...
            if (options.fields) {
                _(options.fields).forEach(function (field, key) {
                    ...
                    child.attr('options', 'options.fields[' + key + ']');
                    child.attr('offset', 'offset');
                    child.attr('horizontal', 'horizontal');
                    child.attr('inline', 'inline');
                    child.attr('model', model);
                    child.attr('name', name);
                    child.attr('form', 'form');
                    child.attr('validation', 'validation["' + name + '"]');
    ...
    return {
        ...
        link: function link($scope, element) {
            ...
            if (options.horizontal) {
                ...
                console.log('form', numeric, offset);
    ...
    return {
        ...
        link: function link($scope, element) {
            ...
            if ($scope.horizontal) {
                var numeric = !_.isArray($scope.offset) && $scope.offset - parseFloat($scope.offset) + 1 >= 0;
                var offset = numeric ? parseInt($scope.offset) : 2;
                console.log('text', $scope.name, numeric, offset);
                $scope.class.wrapper = 'col-sm-' + (12 - offset);
